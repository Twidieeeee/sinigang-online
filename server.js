...
function score(r,idx){
 ...
 let s=0,b=[];
 const baboy=c.Baboy||0;
 const hipon=c.Hipon||0;
 if(baboy===2&&hipon===2){
   s-=10;
   b.push('Baboy + Hipon: 2 Baboy + 2 Hipon = -10');
 }else if(baboy===2&&hipon===1){
   s+=8;
   b.push('Baboy + Hipon: 2 Baboy + 1 Hipon = 8 (10 - 2)');
 }else if(baboy===1&&hipon===2){
   s+=5;
   b.push('Baboy + Hipon: 1 Baboy + 2 Hipon = 5 (8 - 3)');
 }else if(baboy===1&&hipon===1){
   b.push('Baboy + Hipon: 1 Baboy + 1 Hipon = 0');
 }else if(baboy===2){
   s+=10;
   b.push('Baboy: 2 × 5 = 10');
 }else if(baboy===1){
 ...
 const ka=c.Kangkong||0;if(ka){let p=ka*3;s+=p;b.push(`Kangkong: ${ka} × 3 = ${p}`)}
 const km=c.Kamatis||0;if(km){let p=km*2;s+=p;b.push(`Kamatis: ${km} × 2 = ${p}`)}
 ...
 let w=r.players.find(p=>p.vp>=WINNING_KANIN);r.winner=w?.name||null;r.phase=w?'gameover':'result';r.lastResult={score:s,threshold:KAKAIN_NA_THRESHOLD,breakdown:b,roundWinner:rw,winner:r.winner};return r.lastResult
}
